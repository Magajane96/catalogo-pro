...
export default async function StorePage({params}:{params:Promise<{slug:string}>}) {
  ...
  const [{data:products},{data:categories}]=await Promise.all([
    supabase.from("products").select("id,name,price,promotional_price,category_id,product_images(url,is_primary)").eq("store_id",store.id).eq("active",true).order("created_at",{ascending:false}),
