  ...
  return <div className="mx-auto max-w-7xl">
    <div className="premium-panel rounded-3xl p-6 sm:p-8">
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div><p className="text-xs font-extrabold uppercase tracking-[.2em] text-brand">Catálogo</p><h2 className="font-display mt-2 text-3xl font-extrabold">Produtos</h2><p className="mt-2 text-slate-500">Gerencie a vitrine, estoque e apresentação comercial da sua loja.</p></div>
        {reachedProductLimit ? <span className="flex h-11 items-center gap-2 rounded-xl bg-slate-200 px-5 text-sm font-extrabold text-slate-500"><Lock size={18}/>Limite atingido</span> : <Link href="/dashboard/produtos/novo" className="flex h-11 items-center gap-2 rounded-xl bg-brand px-5 text-sm font-extrabold text-white shadow-lg shadow-emerald-700/20 transition hover:-translate-y-0.5"><Plus size={18}/>Novo produto</Link>}
      </div>
    </div>

    <section className={`mt-7 rounded-3xl border p-5 shadow-sm ${reachedProductLimit ? "border-amber-200 bg-amber-50" : "border-emerald-100 bg-emerald-50"}`}>
    ...
    <form className="mt-5 grid gap-3 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-[1fr_220px_auto]">
      <label className="relative">
        <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
        <input name="q" defaultValue={search} placeholder="Buscar produto ou SKU" className="input-premium h-11 w-full rounded-2xl pl-11 pr-4 text-sm font-semibold" />
      </label>
      <select name="filter" defaultValue={selectedFilter} className="input-premium h-11 rounded-2xl px-3 text-sm font-bold">
...
function StockCard({ label, value, className }: { label: string; value: number; className: string }) {
  return <div className="premium-card rounded-3xl p-5">
