...
declare
  ...
  v_item jsonb; v_product record; v_quantity integer; v_phone text; v_variant_name text; v_remaining_stock integer; v_notes text;
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    values(
      ...
      jsonb_build_object('order_id',v_order_id,'order_number',v_order_number,'product_id',v_product.id,'product_name',v_product.name,'quantity_removed',v_quantity,'remaining_stock',v_remaining_stock)
    );
  end loop;
