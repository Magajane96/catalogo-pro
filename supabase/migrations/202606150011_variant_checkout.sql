do $$
begin
  if not exists(select 1 from pg_policies where schemaname='public' and tablename='product_options' and policyname='options_public') then
    create policy "options_public" on public.product_options
      for select
      using(exists(select 1 from public.products p join public.stores s on s.id=p.store_id where p.id=product_id and p.active and s.published));
  end if;

  if not exists(select 1 from pg_policies where schemaname='public' and tablename='product_option_values' and policyname='option_values_public') then
    create policy "option_values_public" on public.product_option_values
      for select
      using(exists(select 1 from public.product_options o join public.products p on p.id=o.product_id join public.stores s on s.id=p.store_id where o.id=option_id and p.active and s.published));
  end if;

  if not exists(select 1 from pg_policies where schemaname='public' and tablename='product_variants' and policyname='variants_public') then
    create policy "variants_public" on public.product_variants
      for select
      using(active and exists(select 1 from public.products p join public.stores s on s.id=p.store_id where p.id=product_id and p.active and s.published));
  end if;
end $$;
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    if v_variant_id is not null then
      select id,name,price_adjustment,stock
        into v_variant
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    if v_variant_id is not null then
      ...
      insert into public.order_items(order_id,product_id,variant_id,product_name,variant_name,quantity,unit_price,total)
      values(v_order_id,v_product.id,v_variant.id,v_product.name,coalesce(v_variant_name,v_variant.name),v_quantity,v_product.unit_price + v_variant.price_adjustment,(v_product.unit_price + v_variant.price_adjustment)*v_quantity);
    else
      update public.products
    ...
    values(
      ...
      jsonb_build_object('order_id',v_order_id,'order_number',v_order_number,'product_id',v_product.id,'product_name',v_product.name,'variant_id',v_variant_id,'quantity_removed',v_quantity,'remaining_stock',v_remaining_stock)
    );
  end loop;
  ...
  if p_status = 'cancelled' and v_order.stock_restored_at is null then
    ...
    loop
      ...
      values(
        ...
        jsonb_build_object('order_id',v_order.id,'order_number',v_order.order_number,'product_id',v_item.product_id,'product_name',v_item.product_name,'variant_id',v_item.variant_id,'variant_name',v_item.variant_name,'quantity_restored',v_item.quantity)
      );
    end loop;
