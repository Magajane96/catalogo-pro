...
declare
  ...
  v_item jsonb; v_product record; v_quantity integer; v_phone text;
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    insert into public.order_items(order_id,product_id,product_name,quantity,unit_price,total) values(v_order_id,v_product.id,v_product.name,v_quantity,v_product.unit_price,v_product.unit_price*v_quantity);
  end loop;
  insert into public.activities(store_id,type,title,metadata) values(p_store_id,'order_created','Novo pedido recebido',jsonb_build_object('order_id',v_order_id,'order_number',v_order_number,'total',v_total));
